...
Based on V5.1_Final + per-task VRF audit + 99/1 reward distribution + no DA + overspend self-protection · Lightning scheme · million TPS architecture
...
Quality issues vs binary judgment: Bitcoin has no "poor quality" transactions. FunAI has "7B pretending to be 70B" quality fraud. → Random auditing needed.
...
MsgAuditResult — Random audit/re-audit results
EndBlocker: audit_rate / reaudit_rate dynamic calculation (per epoch) + block reward distribution (99% inference + 1% verification/audit) + Worker exit processing + committee rotation
...
FunAI's security model = Lightning (signature is evidence, expiry is safety) + every task verified + 10% random audit. 90% instant settlement, 10% settled after audit. Not OP's "optimistic assumption."
...
Auditor	GPU runs teacher forcing (review)	GPU	Per-task VRF purely random selects 15-20 candidates; first 3 to submit results count
...
VerifyResult
Auditors request data from original nodes via P2P. 5 nodes each retain data; normal audits complete within tens of seconds (timeout upper limit 12 hours), data is sufficient.
...
Audit	0.0	task_id || post_verification_block_hash	Equal probability random
Re-audit	0.0	task_id || post_audit_block_hash	Equal probability random
...
Why audit uses pure random: Pure random is the safest; whale probability of controlling auditors drops by 3400x.
...
T=13.7s   Next block arrives → block_hash determined
          Proposer computes: VRF(task_id + block_hash) < audit_rate?
          ├── NO (90%) → CLEARED → enters BatchSettlement → settlement and distribution (~15s to account)
          └── YES (10%) → PENDING_AUDIT → triggers audit
T≈25s     Audit complete (if selected) → CLEARED → settlement and distribution
User experience: ~0.6s to see the first token, ~8.3s inference complete. 90% of tasks ~15s instant settlement. 10% audited ~25s settlement. All asynchronous in background; user doesn't notice.
...
8.4 Verifier and Auditor Sampling Check
After verifiers and auditors complete the logits comparison (V5.2's existing step), if temperature > 0, they additionally perform sampling verification:
...
Detection rate depends on whether the cheated positions are selected by VRF. 5 sampling positions / total generated tokens = sampling coverage rate. Tampering with 1 token in 500-token output → single detection probability = 5/500 = 1%. With 3 verifiers independently verifying, probability of at least one detecting ≈ 3%. Combined with 15% audit rate, final evasion probability ≈ 97% x 85% ≈ 82%.
...
3/3 PASS → VERIFIED (SUCCESS). Any FAIL → VERIFIED (FAIL). Next block's VRF determines whether to audit.
...
Audit fund	0.5%
Total	100.0%
FAIL task (user only charged 5% fee): Worker gets nothing; verifiers each 1.5% + audit fund 0.5% = 5%.

Audit fund is settled each epoch, no accumulation or deficit. Audit pool total = all settled transactions x original fee x 0.5% (both SUCCESS and FAIL tasks contribute 0.5%; for FAIL, this 0.5% comes from the 5% charged to user). Computed once per epoch in EndBlocker: audit person-times = audited transactions x 3 people + re-audited transactions x 3 people. Per person-time fee = audit pool total / audit person-times.
...
8. Next block arrives → nodes compute VRF(task_id + block_hash) < audit_rate?
   - NO → CLEARED → complete evidence broadcast via P2P → Proposer packages into BatchSettlement
   - YES → PENDING_AUDIT → triggers audit → after audit completion check re-audit VRF → 99% CLEARED / 1% PENDING_REAUDIT
...
Lazy-copy Worker and say PASS	0.015 FAI	Jointly jailed during audit
...
10.1 Design Principle: No Settlement Before Audit
Tasks selected for audit are not settled until audit is complete (cheaters can't get money). Tasks not selected are settled instantly (90% of tasks arrive in ~15s).

Why not escrow everything for 24h: The vast majority of tasks are normal; no need to freeze funds. Per-task VRF audit can only determine whether to audit in the block after verification completes; Workers don't know beforehand and cannot exploit this.
...
State	Meaning
VERIFIED	Verification complete (3/3 PASS or with FAIL), waiting for next block to determine whether to audit
CLEARED	Not audited OR audit/re-audit passed → can settle and distribute
PENDING_AUDIT	Selected for audit → waiting for audit results → no settlement
PENDING_REAUDIT	Selected for re-audit after audit → waiting for re-audit results → no settlement
FAILED	Audit/re-audit failed → no settlement → jail
VERIFIED (verification complete, carrying original result: SUCCESS or FAIL)
    │
    ▼ Next block arrives → VRF(task_id + block_hash) < audit_rate?
    │
    ├── NO (90%) → CLEARED → enters BatchSettlement → settled per original result (~15s)
    │                         ├── Original SUCCESS → user charged 100%, Worker 95% + verifiers 4.5% + audit pool 0.5%
    │                         └── Original FAIL → user charged 5%, verifiers 4.5% + audit pool 0.5%, Worker jail
    │
    └── YES (10%) → PENDING_AUDIT → triggers audit
                        │
                        ▼ Audit complete → VRF(task_id + post_audit_block_hash) < reaudit_rate?
                        │
                        ├── NO (99%) → Audit result takes effect
                        │     ├── Confirmed (audit consistent with original) → CLEARED → settled per original result
                        │     └── Overturned (audit inconsistent with original)
                        ...
                        │
                        └── YES (1%) → PENDING_REAUDIT → triggers re-audit
                              │
                              ├── Re-audit confirms → audit result takes effect (same as above)
                              └── Re-audit overturns → original auditors jail
                                    ├── Audit PASS overturned → FAILED → no settlement
                                    └── Audit FAIL overturned → CLEARED → settled per original verification result
90% of tasks arrive in ~15s. ~9.9% arrive ~25s after audit. ~0.1% arrive ~40s after re-audit. Workers get paid instantly the vast majority of the time.
...
1,000 entries ≈ 200KB
Only tasks in CLEARED state are packaged by the Proposer into BatchSettlement. PENDING_AUDIT / PENDING_REAUDIT tasks wait until audit/re-audit completes and they become CLEARED before being packaged. After on-chain processing, only the merkle_root is persisted. Does not depend on an external DA layer.
...
10.6 Audit Overturn Handling
Audit overturns only occur on PENDING_AUDIT tasks (these tasks were not settled before audit; there is no "clawback" issue):

Audit overturn SUCCESS→FAIL (Worker cheated + verifiers were lazy):

No settlement (money hasn't been paid out)
Worker.jail_count += 1
Original PASS verifiers each jail_count += 1
Audit overturn FAIL→SUCCESS (verifiers maliciously framed Worker):

Distribute as success: Worker += 95%, verifiers each += 1.5%, audit fund += 0.5%
Original FAIL verifiers each jail_count += 1
Worker unaffected
10.7 Re-Audit Overturn Handling
Re-audit overturns only occur on PENDING_REAUDIT tasks (audit complete but not settled; there is no "clawback" issue):

Re-audit overturn audit PASS→FAIL (auditors lazily passed problematic work):

No settlement (money hasn't been paid out)
Worker.jail_count += 1
Original PASS verifiers each jail_count += 1
Original PASS auditors each jail_count += 1
Re-audit overturn audit FAIL→PASS (auditors maliciously overturned correct results):

Settle per original verification result (Worker innocent → settle as success; Worker actually FAIL → settle as failure)
Original FAIL auditors each jail_count += 1
Re-audit confirms audit result (no overturn):

Audit PASS + re-audit PASS → CLEARED → normal settlement
Audit FAIL + re-audit FAIL → FAILED → no settlement → Worker jail (+ verifier jail if applicable)
Note: CLEARED instantly-settled tasks will not be overturned by audit/re-audit. If a Worker sent fake content to a user, the user SDK submits a FraudProof to handle it (Section 12.4), which requires clawing back already-settled funds.

10.8 User Experience
SDK handles silently. User sends request, receives streaming results; all background settlement/audit is imperceptible.
...
Source 1 (Audit overturn SUCCESS→FAIL): Worker actually cheated but verifiers let it pass → Worker jail + original PASS verifiers jail
Source 2 (Audit overturn FAIL→SUCCESS): Worker had no issues but verifiers misjudged/maliciously judged FAIL → original FAIL verifiers jail
...
Source 4 (Re-audit overturn audit PASS→FAIL): Auditors lazily passed problematic work → original PASS auditors jail + Worker jail + original PASS verifiers jail
Source 5 (Re-audit overturn audit FAIL→PASS): Auditors maliciously overturned correct results → original FAIL auditors jail
Division of labor:

Layer	Catches what	How triggered
Verification	Worker cheating (real-time)	Verification FAIL → jail after CLEARED settlement
Audit	Worker cheating + verifier laziness (all PASS slipped through)	Audit overturn → Worker jail + verifier jail
Audit	Verifier malice (framing Worker)	Audit overturn FAIL→SUCCESS → verifier jail
Re-audit	Auditor laziness (let problems pass)	Re-audit overturn PASS→FAIL → auditor jail + Worker jail + verifier jail
Re-audit	Auditor malice (overturned correct results)	Re-audit overturn FAIL→PASS → auditor jail
...
50 consecutive successful completions → jail_count resets to zero. The definition of "1 task" varies by role: Worker = 1 inference, verifier = 1 verification, auditor = 1 audit, Leader = 1 dispatch epoch, Proposer = 1 block production. All roles share the same counter and the same progression mechanism.

Worker, Leader, Proposer, verifier, auditor all share the same jail progression mechanism.
...
Verification false positive (original FAIL but audit overturns): Audit overturn = correction mechanism, Worker unharmed. False-positive verifiers get jailed.

Note: Only the 10% of FAIL tasks that are audited have a chance of being corrected. The 90% of FAIL tasks that are not audited are settled per original result → Worker directly jailed. Therefore, false positive cost is backstopped by the 4/5 matching rule: under the 4/5 rule, the false positive rate is approximately 0.003% (when single-position epsilon error is 0.1%, the probability of >=2 positions simultaneously having errors ≈ C(5,2) x 0.001^2 ≈ 0.001%), about 30 false positive FAILs per 1 million daily inferences. Of these, 27 (90%) result in Workers bearing a first-time jail of 10 minutes; 3 (10%) are overturned by audit with Workers unharmed.

Audit false positive: Majority false positive = one in a million. Three consecutive times to slash = impossible.

Part 13: Random Audit
13.1 Why It's Needed
Verifier VRF's 0.5 random reduces collusion probability but not to zero. Without audits → zero-risk collusion for profit. Large mining pools would absolutely do this.

Auditing FAIL results is also needed: without auditing FAIL → malicious verifiers can frame honest Workers at zero cost.

The sole reason audits exist = fear of verifier laziness.

Layer	Deters whom	Logic
Verification	Workers	You cheat, I FAIL you
Audit	Verifiers	You're lazy, I catch you and jointly jail you
Re-audit	Auditors	You're lazy, I check again
13.2 Four Core Principles
Whether to audit is only known after each task's verification is complete (unpredictable)
Audit rate dynamically adjusts following network health (not fixed)
More work means more audits with no upper limit (automatically fair)
No settlement before audit (cheaters can't get money)
13.3 Why Not "N Times Per Worker Per Day"
Fatal vulnerability: Worker knows it's been audited for the day → remaining orders are 100% safe → cheat worry-free. Verifiers also know → be lazy worry-free. Fixed count = telling Workers when it's safe.

Per-task 10% audit: Task 1 is 10%. Task 2 is still 10%. Task 100 is still 10%. Each task is independently random. There is no "done being audited so it's safe."

13.4 Audit Trigger Mechanism (Per-Task VRF)
After verification completes, the Proposer computes in the next block:

VRF(task_id + post_verification_block_hash) < audit_rate?
...
YES (10%) → PENDING_AUDIT → triggers audit → no settlement until audit completes
13.5 Audit Rate Dynamic Adjustment
Not fixed. Automatically computed each epoch.

base_rate = 10%.

audit_rate = base_rate × (1 + 10 × recent_fail_ratio + 50 × recent_audit_fail)
recent_fail_ratio = failed settlement ratio in the previous epoch. recent_audit_fail = audit failure ratio in the previous epoch. Floor 5%. Ceiling 30%.

When the network is normal, audit_rate ≈ 10%. Someone cheats → fail_ratio rises → audit_rate rises → more checks. Large-scale cheating → audit_rate rises to 30%.

Automatically fair by task count: Worker A does 100 tasks/day x 10% = 10 audited. Worker B does 10,000 tasks/day x 10% = 1,000 audited. More work means more audits. No manual tiering needed.

13.6 Auditor Process
VRF purely random (alpha=0.0) selects 15-20 audit candidates (excluding original Worker and original verifiers).

First check P2P: already 3 audit results for this task_id → don't audit → save GPU
...
First 3 to submit results receive audit fees. Later ones do not.
On-chain judgment (after MsgAuditResult goes on-chain):

Original result	Audit majority	Handling
SUCCESS	Audit PASS (>=2/3)	Confirmed → CLEARED → settlement
SUCCESS	Audit FAIL (>=2/3)	Overturned → FAILED → no settlement → Worker jail + verifier jail
FAIL	Audit PASS (>=2/3)	Overturned → CLEARED → settle as success → original FAIL verifiers jail
FAIL	Audit FAIL (>=2/3)	Confirmed → CLEARED → settle as failure → Worker jail
Note: The above handling is executed after the re-audit VRF check. After audit completion, first check VRF(task_id + post_audit_block_hash) < reaudit_rate?; 99% no re-audit → immediately execute per the table above; 1% re-audit → PENDING_REAUDIT → wait for re-audit results before executing (re-audit can overturn audit conclusions, see Section 14.2).
...
Laziness profit = saving 0.4 seconds of GPU for inference = 0.4 x 0.546 = 0.22 FAI. Laziness risk = 10% audited → jointly jailed 10 minutes (first time) → loss of 600 x 0.01025 = 6.15 FAI (daily average income per second). Expected loss = 10% x 6.15 = 0.615 FAI.
...
13.8 Audit Rate Comparison
audit_base_rate	Laziness expected	On-chain overhead	CometBFT utilization	GPU proportion
...
Slip-through = p^3 x [(1 - audit_rate) + audit_rate x p^3]
...
At 100k Workers (10% audit rate): ~300k MsgAuditResult per day = 3.47 tx/s. CometBFT utilization 0.17%. Approximately zero.

GPU overhead: ~1 second of audit teacher forcing per GPU per day. Negligible.

13.11 Audit Timeout
Initial audit: 12-hour timeout. Re-audit: 24-hour timeout.

Timeout handling = original verification result takes effect:

VERIFIED (SUCCESS) + audit timeout → CLEARED, settle as SUCCESS
VERIFIED (FAIL) + audit timeout → CLEARED, settle as FAIL
Timeout is most likely due to network issues or auditors not responding; Workers should not be additionally penalized for this. The original verification has 3 verifiers backing it, providing sufficient credibility as a fallback.

Re-audit timeout follows the same principle: original audit result takes effect.

Part 14: Re-Audit Mechanism
14.1 Why It's Needed
Audit is the second line of defense. Re-audit is the third. Three layers are 128x safer than two layers, costing 1% more in audit volume.
...
14.2 Mechanism
After audit results come in, the next block computes:

VRF(task_id + post_audit_block_hash) < reaudit_rate?
reaudit_rate dynamically adjusts, same principle as audit rate:

reaudit_base_rate = 1%

reaudit_rate = reaudit_base_rate × (1 + 10 × recent_audit_overturn_ratio + 50 × recent_reaudit_overturn)
recent_audit_overturn_ratio = audit overturn ratio in the previous epoch. recent_reaudit_overturn = re-audit overturn ratio in the previous epoch. Floor 0.5%. Ceiling 5%.

When the network is normal, reaudit_rate ≈ 1%. If auditors are massively lazy causing the overturn rate to rise → reaudit_rate automatically increases → more checks.

NO (99%) → Audit result takes effect (PASS → CLEARED, FAIL → FAILED + jail)
YES (1%) → PENDING_REAUDIT → triggers re-audit → VRF purely random selects 15-20 new candidates → first 3 to submit results count
Re-auditors don't know they are re-auditors. Same operations as audit. Re-audit VRF is also purely random. Also uses post-audit block_hash. Unpredictable.

Re-audit results can further overturn the initial audit conclusion. Upon overturn, original auditors are jailed (same progression mechanism):

Original audit result	Re-audit majority	Handling
Audit PASS (>=2/3)	Re-audit FAIL (>=2/3)	Overturn → FAILED → no settlement → original PASS auditors jail + Worker jail + verifier jail
Audit PASS (>=2/3)	Re-audit PASS (>=2/3)	Confirmed → CLEARED → settlement
Audit FAIL (>=2/3)	Re-audit PASS (>=2/3)	Overturn → settle per original verification result → original FAIL auditors jail
Audit FAIL (>=2/3)	Re-audit FAIL (>=2/3)	Confirmed → FAILED → no settlement → Worker jail
14.3 On-Chain Overhead
Normal (1%): ~3,000 MsgAuditResult per day = 0.035 tx/s. Approximately zero.
...
1% distributed by verification/audit count. Verifiers and auditors split evenly by the number of verifications/audits within the previous epoch.

Data source = on-chain records from the previous epoch (current epoch's BatchSettlement may not be fully on-chain yet). Only counts CLEARED contributions. PENDING_AUDIT doesn't count. FAILED doesn't count. Cheating tasks that fail audit → no settlement → don't count as contribution → don't share rewards.

Why allocate 1% to verification/audit:
...
Plus random audit (alpha=0.0) → collusion expected profit is negative.
...
20%    0.0000512%            0.5
50%    0.2%                  2,000
18.3 Per-Task Audit Security Gain
Per-task VRF audit additionally provides:

No settlement before audit → cheating tasks get no money → zero revenue (safer than escrow clawback)
90% instant settlement → Worker cash flow is good (faster than 24h escrow)
No safety window → each task has independent 10% probability → no "done being audited so I can cheat worry-free"
Bidirectional correction: audit overturn SUCCESS→FAIL doesn't settle (money not paid out); overturn FAIL→SUCCESS settles as success
...
x/settlement	User balance management / BatchSettlement / per-task audit / FraudProof
...
MsgAuditResult	Packaged by Proposer	Audit/re-audit results (auditors broadcast P2P, Proposer picks up)
MsgFraudProof	User SDK	Report fake content
MsgUnjail	Jailed node	Unjail (Worker, Leader, Proposer, verifier, auditor can all submit)
...
reward_verification_weight	1% (with inference, by verification/audit count)
reward_inference_formula	80% fee + 20% count
reward_verification_formula	Split evenly by verification/audit count
...
Settlement
Parameter	Value
settlement_model	Per-task VRF audit (no settlement before audit, non-audited instant settlement)
settlement_states	VERIFIED → CLEARED (90%) / PENDING_AUDIT (10%) → CLEARED / PENDING_REAUDIT (0.1%) / FAILED
success_settlement	user charged 100%: Worker 95% + verifiers 4.5% + audit pool 0.5%
fail_settlement	user charged 5%: verifiers 4.5% + audit pool 0.5% + Worker jail
...
reaudit_overturn_pass_to_fail	No settlement (PENDING_REAUDIT money not paid) + original PASS auditors jail + Worker jail + verifiers jail
reaudit_overturn_fail_to_pass	Settle per original verification result + original FAIL auditors jail
...
Audit
Parameter	Value
audit_trigger	Per-task VRF (next block after verification completion, VRF parameters see "Dispatch and Election" table)
audit_base_rate	10%
audit_rate_min	5%
audit_rate_max	30%
audit_rate_formula	base_rate × (1 + 10 × recent_fail_ratio + 50 × recent_audit_fail)
audit_scope	Both SUCCESS and FAIL tasks can be selected
audit_candidates	15-20
audit_required	3 (first 3 to submit count)
audit_match_threshold	2/3 majority
audit_fund_ratio	0.5%
audit_fund_type	Settled each epoch
audit_timeout	12 hours
audit_timeout_action	Original verification result takes effect (CLEARED)
audit_data_source	P2P request from original nodes (nodes retain for 7 days)
Re-Audit
Parameter	Value
reaudit_trigger	Per-task VRF (next block after audit completion, VRF parameters see "Dispatch and Election" table)
reaudit_base_rate	1%
reaudit_rate_min	0.5%
reaudit_rate_max	5%
reaudit_rate_formula	reaudit_base_rate × (1 + 10 × recent_audit_overturn_ratio + 50 × recent_reaudit_overturn)
reaudit_candidates	15-20
reaudit_required	3
reaudit_timeout	24 hours
reaudit_timeout_action	Original audit result takes effect
Penalty
Parameter	Value
jail_trigger_sources	Verification FAIL confirmation + audit overturn + re-audit overturn (same counter)
...
jail_applies_to	Worker, Leader, Proposer, verifier, auditor (shared)
...
audit_vrf_alpha	0.0 (pure random)
audit_vrf_seed	task_id || post_verification_block_hash
reaudit_vrf_alpha	0.0 (pure random)
reaudit_vrf_seed	task_id || post_audit_block_hash
...
2	"Verifier laziness is a pseudo-problem" depends on the premise that laziness rate p < 30%	Risk reduced with audit joint jail + audit overturn; not addressing for now
...
3	Settlement model	Instant settlement (BatchSettlement distributes directly)	Per-task VRF audit: 90% instant settlement + 10% settled after audit	No settlement before audit prevents cheaters getting money; 90% instant preserves Worker cash flow
4	FAIL also audited	Only audit SUCCESS tasks	Both SUCCESS and FAIL audited	Not auditing FAIL → malicious verifiers can frame Workers at zero cost
5	Remove DA	Details stored in DA layer	Details directly in BatchSettlement transaction body	Remove external dependency; nodes retaining 7 days is sufficient for audit
6	Data retention	Undefined	Participating nodes retain 7 days	Audit data source, replacing DA
7	BatchSettlement unlimited	1-2 per block	Unlimited as standard Msg	Original design's settlement TPS was only 40k
8	Audit joint liability simplified	epsilon/20 logits distance comparison	Audit FAIL → all original PASS verifiers jail	epsilon/20 false-positives same GPU model
...
14	Audit timeline	Unscheduled daily	Audit timeout 12h + re-audit timeout 24h	Audit triggers in next block after verification; timeout as fallback
15	DA layer	TBD (Celestia/Avail/custom)	Not needed	Local node retention of 7 days as replacement
16	Block reward distribution	100% by inference contribution	99% inference + 1% verification/audit	Without allocation, verification loses money (net loss 0.033 FAI/task); allocating 1% makes verification earn 12% more per second than inference
17	Audit model	N times per Worker per day + 12h scheduling	Per-task VRF random audit (base 10%, dynamic 5-30%)	Fixed count has safety window; per-task is independently random per task with no window
18	Settlement model	All instant settlement (no audit blocking)	90% instant settlement + 10% settled after audit (no settlement before audit)	Per-task VRF audit only blocks the 10% selected; 90% instant preserves cash flow
19	Audit candidate count	8-10	15-20	Increased redundancy to ensure 3 results are gathered quickly
20	Audit rate adjustment	Fixed	Dynamic: base_rate × (1 + 10 × fail_ratio + 50 × audit_fail), 5-30%	Follows network health; low overhead when normal, auto-escalates when cheating occurs
21	Settlement state machine	None	VERIFIED → CLEARED/PENDING_AUDIT → CLEARED/FAILED	Clear state transitions; no settlement before audit
...
24	Re-audit overturn punishment	Undefined	Re-audit overturn → original auditors jail (same progression)	Fill in "re-audit deters auditors" punishment implementation
25	PENDING_REAUDIT state	None	Post-audit re-audit selection → PENDING_REAUDIT → no settlement until re-audit complete	Consistent with audit: no settlement before re-audit, avoiding clawback
...
31	Running threshold worker count	Only looks at stake ratio	installed_worker_count >= 10	Can't gather enough people for inference+verification+audit
...
34	State machine distinguishes confirm/overturn	Audit FAIL uniformly → FAILED	Confirm original FAIL → CLEARED settle as failure; overturn SUCCESS→FAIL → FAILED no settlement	Original state machine oversimplified; two cases require different handling
35	MsgUnjail scope	Worker/Leader	All jailed roles can submit	Jail applies to 5 roles; Unjail should too
36	jail_count reset definition	"50 tasks" undefined	Defined by role: Worker=inference, verifier=verification, auditor=audit, Leader=epoch, Proposer=block production	Non-Worker roles can't use "50 inferences"
...
38	VRF scenario completion	Only defined dispatch/verification/audit 3 types	Added re-audit/Leader election/Validator committee, total 6 types	All VRF elections need explicit seed and alpha
...
47	InferReceipt adds final_seed + sampled_tokens	Only worker_logits + result_hash	Added final_seed(bytes32) + sampled_tokens([uint32;5])	Verifiers/auditors need these fields for sampling verification
