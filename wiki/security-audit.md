...
Tracker for all security audit findings across the FunAI Chain project.

## Security Audit (KT)

Baseline commit: `ce87883` (2026-04-03). Source: [FunAI_Security_Audit_Findings_KT.md](../docs/FunAI_Security_Audit_Findings_KT.md)

| ID | Description | Severity | Status | Resolution |
|----|-------------|----------|--------|------------|
| A1 | `ProcessWithdraw` doesn't check frozen balance | CRITICAL | FIXED | Commit `2cb3dab` -- uses `AvailableBalance()` |
| A2 | Timeout fee not transferred to audit fund | NOT A BUG | Closed | Audit fund = implicit difference between user payment and Worker/verifier payouts |
...
## Dispatch Audit Fix Checklist

Baseline commit: `599fcd7` (2026-03-30). Source: [Dispatch_Audit_Fix_Checklist.md](../docs/Dispatch_Audit_Fix_Checklist.md)

| ID | Priority | Description | Scope |
|----|----------|-------------|-------|
| D1 | P0 | SDK retry timeout hardcoded 5s -- make configurable with 30s default | ~30 lines |
| D2 | P1 | `AuditResponse` has no signature -- add secp256k1 sig | ~30 lines |
