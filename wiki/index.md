...
| [V6 Batch-Replay Design](../docs/protocol/FunAI_V6_BatchReplay_Design.md) | P0 research-stage design (2026-04-21): fix C0's batched-vs-single logits drift by having the Verifier replay the Worker's exact per-step batch schedule instead of running single-request teacher forcing. Rests on claims A1 (engine can be driven to replay) and A2 (cross-hardware bit-exactness). Validated ahead of protocol rewrite via the PoC at `scripts/v6_replay/` (transformers-based, 3-phase plan). 12 design items total including Leader-side request bundling (item #12, 500 ms Leader window delivers actual batched work to Workers; also reduces C2 adversarial-partner attack surface). Remaining reviewer blockers: compute amplification, log forgery via fabricated partner tasks. | V6_BatchReplay_Design + review |
